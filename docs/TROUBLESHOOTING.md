...
### [08] SPEEDTEST exits immediately or shows an error

- **[08]** uses **speedtest-cli**. If it's missing, the menu will try to install it (pip or apt) and then ask you to press Enter. If install failed, it will suggest:  
...
- User lists are built from current system accounts and the centralized `$HOME/users.db`. If you deleted a user at the system level (e.g. `userdel`), they may still have a record in `$HOME/users.db` until the manager updates it (e.g. via "Remove user" / "Expiration manager").
...
### "Current version: v0" and "Latest version: v0" in [19] UPDATE SCRIPT
...
- Fix: ensure outbound HTTPS works and **wget**/ **curl** are installed. Then run **[19]** again, or set version manually as under "Script says Could not fetch version file" above.
...
- If you run the menu from a copy that wasn't installed by the installer (e.g. only partial files under `/bin`), colors may be missing.  
  Re-run the installer or **[19] UPDATE SCRIPT** so all modules (including `colors`) are placed in `/bin` or `/etc/SSHPlus` as intended.

---

## Server settings [13]

### Root password change fails

- You must run the menu as **root**. The script uses `chpasswd` which requires root privileges.
- If you get "Failed to change root password", verify you're running as root with `whoami` or `id`.

### Timezone change doesn't persist

- The script uses `timedatectl` on systemd systems. If `timedatectl` isn't available, it falls back to symlinking `/etc/localtime`.
- Verify the timezone file exists: `ls /usr/share/zoneinfo/YOUR/TIMEZONE`
- On some minimal containers, timezone data may not be installed: `apt install -y tzdata`
